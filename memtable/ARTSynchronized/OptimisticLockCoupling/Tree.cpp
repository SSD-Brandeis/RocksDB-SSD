...
namespace ART_OLC {
    ...
    bool Tree::lookupRangeReverse(const Key &start, TID result[],
                                std::size_t resultSize, std::size_t &resultsFound, ThreadInfo &threadEpocheInfo) const {
        ...
        std::function<void(N *, uint8_t, uint32_t, const N *, uint64_t)> findStart = [&copy, &start, &findStart, &toContinue, this](
            ...
            switch (prefixResult) {
                ...
                case PCCompareResults::Equal: {
                    ...
                    std::tuple<uint8_t, N *> children[256];
                    uint32_t childrenCount = 0;
                    v = N::getChildren(node, 0, 255, children, childrenCount);
                    for (int32_t i = childrenCount - 1; i >= 0; --i) {
                        const uint8_t k = std::get<0>(children[i]);
                        N *n = std::get<1>(children[i]);
                        if (k == startLevel) {
        ...
        while (true) {
            ...
            switch (compareResult) {
                ...
                case PCCompareResults::Smaller: {
                    uint8_t startLevel = (start.getKeyLen() > level) ? start[level] : 0;
                    if (compareResult == PCCompareResults::Smaller) {
                        startLevel = 255;
                    }
                    
                    uint8_t endLevel = 0;
                    if (startLevel != endLevel || compareResult == PCCompareResults::Smaller) {
                        std::tuple<uint8_t, N *> children[256];
                        uint32_t childrenCount = 0;
                        v = N::getChildren(node, 0, startLevel, children, childrenCount);
                        for (int32_t i = childrenCount - 1; i >= 0; --i) {
                            const uint8_t k = std::get<0>(children[i]);
                            N *n = std::get<1>(children[i]);
                            if (compareResult == PCCompareResults::Equal && k == startLevel) {
                                findStart(n, k, level + 1, node, v);
                            } else {
                                copy(n);
                            }
