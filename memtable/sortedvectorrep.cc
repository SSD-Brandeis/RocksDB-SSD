...
//
//  SortedVectorRep is a MemTableRep implementation that uses a sorted vector.
//  It is useful for workloads where ingestion is heavy and lookups are
//  rare.
//
#include <algorithm>
#include <memory>
...
void SortedVectorRep::Get(const LookupKey& k, void* callback_args,
  ...
  if (immutable_) {
    vector_rep = this;
    local_bucket = bucket_; 
  } else {
    vector_rep = nullptr;
    local_bucket.reset(new Bucket(*bucket_));  // make a copy
  }

  SortedVectorRep::Iterator iter(vector_rep, local_bucket, compare_);
  rwlock_.ReadUnlock();
